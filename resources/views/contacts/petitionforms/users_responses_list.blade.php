                    ...
                    <div class="container-fluid">
                        <div class="row align-items-center">
                            <div class="col-md-12 col-12">
                                <h2 class="breadcrumb-title">Petition Supporters</h2>
                                <nav aria-label="breadcrumb" class="page-breadcrumb">
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item"><a href="{{route('dashboard')}}">Dashboard</a></li>
                                        <li class="breadcrumb-item"><a href="{{route('petitioncustom.dashboard')}}">Petitions Dashboard</a></li>
                                        <li class="breadcrumb-item active" aria-current="page">Petition Supporters</li>
                                    </ol>
                                </nav>
                            </div>
                        </div>
...
<script src="{{ asset('assets/js/simcify.min.js') }}"></script>
    ...
    $(document).ready(function(){
      $.ajax({
       url:"{{route('petitioncustomForm.sidebar',!empty($form->id) ? encrypted_key($form->id,'encrypt') :0)}}?sidebar=form_users_responses",
   ...
   $(document).on("click", ".destroypetitioncustomuser", function(){
...
<script type="text/javascript">
        $(function () {
            var table = $('#myTableres').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('petitioncustomForm.responseUsers',encrypted_key($id,'encrypt')) }}",
                columns: [
