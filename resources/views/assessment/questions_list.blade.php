    ...
    <div class="col-lg-8 order-lg-1">
        ...
        <span class="btn-inner--text"><i class="fas fa-reply"></i></span>
             ...
             @endif             
                ...
                <div class="card-body">
                  <form class="simcy-form indexing-form fields-holder" action="{{ route('assessmentQuestion@indexing') }}" data-parsley-validate="" loader="false" method="POST" enctype="multipart/form-data">
                     @CSRF
                        ...
                        <div class="input-field-item border border-light mb-2 cursor-default" style="cursor: pointer;">
                           ...
                           <div class="row mt-3">
                               ...
                               <div class="col-md-4">
                                   <h6 class="text-muted">Field Label <span class="badge badge-primary badge-xs badge-pill">{{ $question->points }} {{__('Points')}}</span></h6>
                               ...
                               <div class="col-md-2">
                                   <h6 class="text-muted">Action</h6>
                                        <a href="{{route('assessmentQuestion.edit',['form_id' => encrypted_key($form->id,'encrypt'), 'id' => encrypted_key($question->id,'encrypt')])}}" data-toggle="tooltip" data-original-title="{{__('Edit')}}">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="javascript::void(0);" class="text-danger px-2 delete_record_model" data-url="{{route('assessmentQuestion.destroy',encrypted_key($question->id,'encrypt'))}}" data-toggle="tooltip" data-original-title="{{__('Delete')}}">
...
<script src="{{ asset('assets/js/tabsmanager.js') }}"></script>
    ...
    $(document).ready(function(){
      $.ajax({
       url:"{{route('assessmentForm.sidebar',!empty($form->id) ? encrypted_key($form->id,'encrypt') :0)}}?sidebar=form_questions",
   ...
   $(document).on("click", ".destroyassessmentquestion", function(){
...
//       $.ajax({
//           url:"{{route('assessmentCategory.view')}}",
//           success:function(data)
//            {
//            $('#assessment_category_view').html(data);
//            }
//      });           
    </script>  
@endpush
